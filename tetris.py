...
class Tetris:
    ...
    def __init__(self, height=30, width=20, block_size=20):
        self.height = height
        self.width = width
        ...
        for y in range(len(piece)):
            for x in range(len(piece[y])):
                if future_y + y > self.height - 1 or self.board[future_y + y][pos["x"] + x] and piece[y][x]:
                    # print("collision hoise")
                    return True
        # print("collison hoise")
        return False
    ...
    def step(self, action, render=True, video=None):
        ...
        if not self.gameover:
            ...
            # print("\n\n\n")
            # print("paisi toke")
            # for i in self.piece: print(i)
            # change the color of the previous piece to a specific color
    ...
    def render(self, video=None):
        if not self.gameover:
            ...
            # board = self.get_current_board_state()
            # img = [[0 for j in range(len(board[0]))] for i in range(len(board))]

            # for i in range(len(board)):
                # for j in range(len(board[i])):
                    # img[i][j] = self.piece_colors[board[i][j]]
        else:
